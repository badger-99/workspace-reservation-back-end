  ...
  describe 'GET /show' do
    context 'a valid request' do
      it 'shows a specific reservation made by the current user' do
        ...
        reservation = Reservation.new(city:, start_date:, end_date:, reservation_cost:, user_id:, workspace_id:)
        reservation.save
  ...
  describe 'POST /create' do
    context 'creating a reservation with valid parameters' do
      it 'creates a reservation' do
        image = fixture_file_upload('app/assets/images/pizza meme.jpg', 'image/jpg')
        workspace = Workspace.create(name: 'workspace2', description: 'for respec testing', price_per_day: 10, image:)
        ...
        expect(json[:reservation][:start_date]).to eq('2024-10-20')
        expect(json[:reservation][:end_date]).to eq('2024-10-24')
        expect(json[:reservation][:workspace_id]).to eq(workspace.id)
      end
    end
